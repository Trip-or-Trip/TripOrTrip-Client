    ...
    <div id="service-container" class="row p-5">
      ...
      <div id="right-container" class="col-lg-2 col-md-3 p-2">
        ...
        <draggable v-model="places">
          <!-- <update-plan-list-item v-for="(place, index) in places" :key="index" :place="place" :index="index" :total="places.length" @removeItem="removePlanItem"></update-plan-list-item> -->
    ...
    <div class="d-flex flex-column mx-auto p-2" style="width: 100%; height: 35em">
      ...
      <div class="plan-container">
        <div id="plan-detail" class="d-flex flex-column align-items-center rounded mx-auto">
          ...
          <input v-model="plan.title" type="text" id="plan-title" class="plan-detail-content align-middle ms-2 mt-2 rounded" style="width: 70%" />
          <br />
          <div class="plan-detail-date d-flex flex-row justify-content-between mb-3" style="width: 70%">
            ...
            <input v-model="plan.sDate" type="date" id="st-date" style="width: 8em; height: 1.8em" class="plan-detail-content plan-detail-start ms-2 me-2 align-middle rounded" />
            <label for="end-date"><strong>도착일</strong></label>
            <input v-model="plan.eDate" type="date" id="end-date" style="width: 8em; height: 1.8em" class="plan-detail-content plan-detail-end ms-2 me-2 align-middle rounded" />
          </div>
...
export default {
  ...
  created() {
    ...
    http
      ...
      .then(({ data }) => {
        this.plan.id = data.article.id;
        this.plan.title = data.article.title;
        this.plan.sDate = data.article.startDate;
        this.plan.eDate = data.article.endDate;
        this.plan.description = data.article.description;

        let placeItem = {};
        for (let i = 0; i < data.places.length; i++) {
          placeItem = {
            place_name: data.places[i].name,
            first_image: data.places[i].imageUrl,
            address_name: data.places[i].address,
            place_url: data.places[i].mapUrl,
            id: data.places[i].placeId,
            y: data.places[i].lat,
            x: data.places[i].lng,
          };
          this.places.push(placeItem);
        }
  ...
  methods: {
    ...
    initMarker() {
      ...
      for (let i = 0; i < this.places.length; i++) {
        var markerInfo = {
          title: this.places[i].place_name,
          latlng: new window.kakao.maps.LatLng(this.places[i].y, this.places[i].x),
          image: this.places[i].first_image,
          addr: this.places[i].address_name,
          // tel: this.places[i].phone,
          mapUrl: this.places[i].place_url,
          placeId: this.places[i].id,
        };
        this.displayMarker(markerInfo, 1);
        bounds.extend(new window.kakao.maps.LatLng(this.places[i].y, this.places[i].x));
      }
    ...
    savePlan() {
      ...
      http
        ...
        .then(({ data }) => {
          if (data === "success") {
            location.href = `/plan/${this.plan.id}`;
          } else {
