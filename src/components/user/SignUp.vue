        ...
        <form id="signup-form" method="POST" action="${root}/user/signup" role="search">
          ...
          <div class="row d-flex justify-content-center my-4">
            <div class="col-10">
              <div
                v-text="checkId"
                id="check-id-result"
                :class="[validId ? 'text-primary' : 'text-danger']"
              ></div>
            </div>
          </div>
          ...
          <div class="row d-flex justify-content-center my-4">
            ...
            <div class="col-4">
              <input
                ...
                class="form-control"
                v-model="emailDomain"
                @change="checkDuplEmail"
                name="emailDomain"
                id="signup-email-domain"
              />
            </div>
            <div class="col-2">
              <button @click="sendEmail" type="button" class="btn submit-btn" v-if="validEmail">
                인증
              </button>
            </div>
          </div>
          <div class="row d-flex justify-content-center my-4">
            <div class="col-10">
              <div
                ...
                :class="[validEmail ? 'text-primary' : 'text-danger']"
              ></div>
            </div>
          </div>
          <div
            v-if="showEmailAuthInput"
          ...
          >
            <div class="col-10">
              ...
              <button
                type="button"
                @click="checkEmailAuth"
                id="check-email-auth-btn"
...
export default {
  ...
  methods: {
    ...
    validValue() {
      if (!this.id) alert("아이디를 입력해 주세요.");
      else if (!this.validId) alert("사용할 수 없는 아이디입니다. 다시 입력해 주세요.");
      else if (!this.password || !this.checkPassword) alert("비밀번호를 입력해 주세요.");
      else if (this.password != this.checkPassword)
        alert("비밀번호가 일치하지 않습니다. 다시 입력해 주세요.");
      else if (!this.name) alert("이름을 입력해 주세요.");
      else if (!this.emailId) alert("이메일 주소를 입력해 주세요.");
      else if (!this.emailDomain) alert("이메일 도메인을 입력해 주세요.");
      else if (!this.validEmail) alert("사용할 수 없는 이메일입니다. 다시 입력해 주세요.");
      else if (!this.emailConfirmed) alert("이메일 인증에 실패하였습니다. 재시도해주세요.");
      else this.signup();
    },
    signup() {
      console.log("회원가입");
    ...
    checkDuplId() {
      ...
      if (this.id.length < 5) {
        ...
        http.get(`/user/id/${this.id}`).then(({ data }) => {
          if (data == 0) {
    ...
    checkDuplEmail() {
      ...
      if (this.emailId && this.emailDomain) {
        http.get(`/user/email/${this.emailId}/${this.emailDomain}`).then(({ data }) => {
          console.log(data);
          if (
            data == 0 &&
            this.emailDomain.split(".").length >= 2 &&
            this.emailDomain.split(".")[1]
          ) {
    ...
    sendEmail() {
      ...
      http.get(`/user/auth/${this.emailId}/${this.emailDomain}`).then((response) => {
    ...
    checkEmailAuth() {
      http
        .get(`/user/check/${this.emailId}/${this.emailDomain}/${this.emailAuthNumber}`)
        .then((data) => {
          console.log(data);
          if (data.data == true) {
